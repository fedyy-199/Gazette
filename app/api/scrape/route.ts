...
const scrapeRequestSchema = z.object({
  ...
  render: z.literal("html").optional(),
});

/**
 * POST /api/scrape
 * Manual scraping endpoint protected by GAZETTE_ADMIN_SECRET.
 * Adheres strictly to AGENTS.md Sections 14, 15, 16.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    // 3. Run the canonical scraping pipeline
    const summary = await runScrapePipeline(options);
    return NextResponse.json(summary, { status: 200 });
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[POST /api/scrape] Unhandled pipeline error:", err);
    return NextResponse.json(
      { error: "Scraping pipeline failed", details: msg },
      { status: 500 }
    );
  }
}
